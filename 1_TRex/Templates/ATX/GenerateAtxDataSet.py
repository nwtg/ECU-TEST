...
from .Utils import (ConvertConditionBlocks, EmptyReportException, FilterSUCCESS, FilterShortName,
                    ...
                    GetConsumedFilesFromJobItem, FindAssociatedFilesForTrace)
...
class GenerateAtxDataSet(object):
    ...
    def __init__(self, reportApi, firstName, firstDate, isPackageExecution):
        ...
        except EmptyReportException as ere:
            raise ere
        except NameError as ne:
            raise ne
        except BaseException as ex:
            import traceback
            print (u'Exception in {name} -> self.__ReportProjectElement(reportElement):'
                   u'\r\n{ex}\n{st}'.format(name=firstName, ex=ex, st=traceback.format_exc()))
    ...
    def __AddTestStepPlotToArchive(self, path, testStepRefPath, packageVerdict):
        '''
        Erfasst die hinterlegten Plots für eine mögliche Archivierung und TEEST-STEP Zuweisung.
        @param path: Dateipfad.
        @type path: str
    ...
    def __MakeTraceFileArchiveFiles(self, refPath, packageVerdict):
        ...
        for eachTraceFile, eachInfo in self.__traceFilesToArchive.items():
            ...
            if filename.lower().endswith(".as3trace"):
    ...
    def __AppendTestConstantElement(self, scope, constantName, constantCategory,
        ...
        checkTextValue = self.FIND_ASCII_CTRL_REG_EXP.sub(u'', checkTextValue)
        ...
        # Entfernt die ASCII-Steuerzeichen aus dem Wert, wenn vorhanden!
        cleanConstantTextValue = self.FIND_ASCII_CTRL_REG_EXP.sub(u'', constantTextValue)
    ...
    def __ConvertPkgToTestCase(self, package, isParameterSetElement, scope,
        ...
        if not testCasePointer:
            ...
            specPkg = OrderedDict([
                ...
                (u'TEST-SETUP-STEPS', packageData[u'setup']),
                (u'TEST-EXECUTION-STEPS', packageData[u'execution']),
                (u'TEST-TEARDOWN-STEPS', packageData[u'teardown']),
            ])
            ...
            # Das Pkg existiert bereits als TestCase und muss nur noch bei erneuten Durchläufen
            # ergänzt werden ...
            # Vergleichen der bestehenden TestStep Struktur mit der aktuell Erzeugten in packageData
            self.__DiffAndAppendTestSteps(testCasePointer[u'TEST-SETUP-STEPS'],
                                          packageData[u'setup'])
            self.__DiffAndAppendTestSteps(testCasePointer[u'TEST-EXECUTION-STEPS'],
                                          packageData[u'execution'])
            self.__DiffAndAppendTestSteps(testCasePointer[u'TEST-TEARDOWN-STEPS'],
                                          packageData[u'teardown'])
        ...
        if self.__refPaths[refPath][u'testCaseReportSpecPointer']:
            if (self.__refPaths[refPath][u'testCaseSpecPointer'][u'SHORT-NAME'] ==
                    u'PreconditionAction'):
                pass
            # Wenn für dieses Package bereits ein Report existiert, dann prüfen ob die REF Pfade
            # (ggf. auch andere Eigenschaften) auch übereinstimmen -> Wenn nicht, müssen die Ref
            # Pfade des neu hinzu kommenden Reports mit denen des bereits vorhandenen überschrieben
            # werden.
            self.__ReworkReportTestSteps(self.__refPaths[refPath][u'testCaseReportSpecPointer'],
                                         packageData[u'reportSteps'])
        ...
        if self.__archive[u'enabled']:
            ...
            for eachTJ in traceJobs:
                for eachTSOriginRef, plots in eachTJ.GetTestStepPlots().items():

                    reportRef = self.__ConvertOriginTSRefToReportTSRef(reportRefPath,
                                                                       eachTSOriginRef)
                    if len(reportRef) > 0:
    ...
    def __CreateRecordings(self, package, reportPkg):
        ...
        for recording in recordings.values():
            ...
            isManualRecording = (recording.GetSource() & 0xFF == Recording.SOURCE_MANUAL
                                 and recording.GetGroupName() != "TC Variables")
    ...
    def __ConvertOriginTSRefToReportTSRef(self, reportTestCaseRef, originTestStepRef):
        '''
        Konvertiert den übergebenen TEST-STEP Origin Ref-Path, in einen TEST-STEP Report Ref-Path
        auf Basis des übergebenen TEST-CASE Report Ref-Paths.
        @param reportTestCaseRef: TEST-CASE Report Ref-Paths des TEST-STEPS
        @type reportTestCaseRef: str
        @param originTestStepRef: TEST-STEP Origin Ref-Path
        @type originTestStepRef: str
        @return: TEST-STEP Report Ref-Path
        @rtype: str
        '''
        result = u""
        testStepPos = originTestStepRef.find(u"step_")
        if testStepPos > 0:
            result = u"{0}/{1}".format(reportTestCaseRef,
                                       originTestStepRef[testStepPos:])
        return result
        ...
        for (key, value) in parameters.items():
            ...
            newTestArgElem = OrderedDict([
                ...
                (u'DESC', {u'L-2': {u'@L': u'DE',
                                    u'#': u'{0}'.format(value.get(u'description', u''))}}),
                (u'TYPE-REF', {
                    u'@DEST': self.__primitiveConstantType[u'@type'],
                    u'#': u'/{0}/{1}'.format(self.__dataTypes[u'SHORT-NAME'],
                ...
                (u'LITERAL-VALUE', {u'TEXT-VALUE-SPECIFICATION': {u'VALUE': value[u'value']}}),
            ])
    ...
    def __SupportsMultipleValues(definition):
        ...
        try:
            ...
            supportsMultipleValues = (isinstance(definition, AttributeTreeValueDef) and
                                      definition.IsMultiChoice())
        except ImportError:
            pass
    ...
    def __GetExecutionTimeInSec(self, package):
        '''
        Ermittelt aus dem übergebenen ReportApi-Package die Duration oder None, wenn diese nicht
        gesetzt ist.
    ...
    def __FilterValueDefaultSuffix(self, value):
        ...
        '''
        # Entfernt die ASCII-Steuerzeichen aus dem Wert, wenn vorhanden!
        value = self.FIND_ASCII_CTRL_REG_EXP.sub(u'', value)
    ...
    def __ReportProjectElement(self, projectElement, parentScope, projectExecLevel,
        ...
        for item in projectElement.IterItems():
            ...
            if itemType == u'Configuration' or itemType == u'ConfigChange':
                ...
                projectAttributes.update(self.__GetProjectAttributes(item))
    ...
    def __HashFile(self, file):
        ...
        if file not in self.__hashCache:
            hash = HashFileContents(file)
            self.__hashCache[file] = hash
