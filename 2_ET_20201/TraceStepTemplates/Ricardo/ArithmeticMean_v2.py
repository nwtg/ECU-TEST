...
class ArithmeticMean_v2(NumpyBasedTraceStep):
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
            ...
            The ArithmeticMean signal gets only value within every specific duration.
            ''').strip()
        ...
        return [
            ParameterDefinition('duration', 'IN', default=0.1,
                                description=textwrap.dedent('''
                                    ...
                                    In seconds.
                                    ''').strip()),
            ParameterDefinition('startTime', 'IN', default=1,
                                description=textwrap.dedent('''
                                    Start trigger time stamp. In seconds.
                                    ''').strip())
            ]
    ...
    def Check(self, parameters):
        ...
        if not isinstance(parameters['startTime'], (int, float)):
            raise TypeError(
                "The parameter 'startTime' has no valid type ({}).".format(
                                                        type(parameters['startTime'])))

        if parameters['startTime'] < 0:
            raise ValueError(
                "The parameter 'startTime' must be greater then zero ({}).".format(
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        for triggerRange in ranges:
            ...
            # create new numpy with the same dimensions with timesIn/valuesIn, but the values should be zeros, otherwise, there is always errors.
            ...
            for i in range(0, timesIn.shape[0]):
                ...
                if round(timesIn[i] - timesOut[k], 11) < round(parameters['duration'], 11):
                    ...
                    if j == 0:
                        timesOut[j] = parameters['startTime']
                    else:
                        timesOut[j] = timesOut[j-1] + parameters['duration']
                    j += 1
                    k = i
                    thenFlag = False
